...
static int exynos_irq_request_resources(struct irq_data *irqd)
{
	...
	printk("Z: %X - %X",irqd->hwirq,bank_type->fld_width[PINCFG_TYPE_FUNC]);
	...
	printk("d->virt_base + reg_con: %X\n",d->virt_base + reg_con);
	
	con = readl(d->virt_base + reg_con);
	con &= ~(mask << shift);
	con |= EXYNOS_EINT_FUNC << shift;
	
	printk("con: %X - %X\n",con,virt_to_phys(d->virt_base + reg_con));
...
static int exynos_eint_wkup_init(struct samsung_pinctrl_drv_data *d)
{
	...
	if (!muxed_data) {
		//dev_err
		printk(dev, "could not allocate memory for muxed_data\n");
		return -ENOMEM;
	}
