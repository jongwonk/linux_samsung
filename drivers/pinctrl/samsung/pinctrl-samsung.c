			     ...
			     struct platform_device *pdev)
{
	...
	for (i = 0; i < ctrl->nr_banks; ++i, ++bdata, ++bank) {
		...
		printk("bdata->nr_pins: %d, bdata->eint_func:%p,bdata->name:%s,bdata->nr_pins:%d\n",bdata->nr_pins,bdata->eint_func,bdata->name,bdata->nr_pins);
	...
	for_each_child_of_node(node, np) {
		...
		for (i = 0; i < d->nr_banks; ++i, ++bank) {
			if (!strcmp(bank->name, np->name)) {
				bank->of_node = np;
				printk("bank->name:%s, pin_base:%X, d->nr_pins:%X \n",bank->name,pin_base,d->nr_pins);
				break;
			}
		}
	}
